...
// The app's aggregates.json is generated by build-recipes.mjs (in build:data,
// so it is always fresh on Vercel). This DuckDB pipeline is the separate
// exploration tier — it writes its rollups to a local, git-ignored file and
// leaves the durable star-schema database for ad-hoc SQL.
const outFile = join(root, 'data/corpus-aggregates.json')
